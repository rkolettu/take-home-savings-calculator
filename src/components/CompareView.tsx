...
function statusForRate(rate: number) {
  if (rate < 0) {
    return { label: 'Deficit', color: 'var(--status-critical)' }
  }
  if (rate < 0.2) {
    return { label: 'Watch out', color: 'var(--status-warning)' }
  }
  if (rate <= 0.5) {
    return { label: 'Healthy', color: 'var(--status-good)' }
  }
  ...
  return (
    <div className="min-h-screen bg-[var(--page)]">
      ...
      >
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4 px-4 py-4 sm:px-6">
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-[var(--text-muted)]">
                Compare up to three metro and income scenarios
              </p>
            </div>
          </div>
          ...
          >
            <a
              href="#"
              className="flex-1 rounded-lg px-3 py-1.5 text-center text-xs font-medium text-[var(--text-secondary)] no-underline transition-colors sm:flex-none"
            >
              Calculator
            </a>
            <a
              href="#compare"
              className="flex-1 rounded-lg bg-[var(--surface-raised)] px-3 py-1.5 text-center text-xs font-medium text-[var(--text-primary)] no-underline sm:flex-none"
            >
              Compare
            </a>
            <a
      ...
      <main className="mx-auto max-w-6xl space-y-6 px-4 py-6 sm:px-6">
        <section
          className="rounded-[22px] border bg-[var(--surface-1)] p-5 sm:p-6"
          style={{ borderColor: 'var(--border)' }}
        >
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="max-w-2xl">
              ...
              <p className="mt-2 text-sm leading-6 text-[var(--text-secondary)]">
                Give each scenario its own metro, salary, filing status, and housing setup. This is a current-year snapshot only — long-term projections are intentionally excluded here.
              </p>
            </div>
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg border bg-[var(--surface-2)] px-3 py-2 text-xs font-semibold text-[var(--text-secondary)] transition-colors disabled:cursor-not-allowed disabled:opacity-45"
              style={{ borderColor: 'var(--border)' }}
            >
              <Plus className="size-4" />
              {scenarios.length >= 3 ? '3 scenario max' : 'Add scenario'}
            </button>
          </div>
        </section>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {scenarios.length > 2 && (
                      <button
                        ...
                        className="rounded-md p-1 text-[var(--text-muted)] transition-colors hover:bg-[var(--surface-2)] hover:text-[var(--status-critical)]"
                      >
              ...
              >
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <label className="block">
                      ...
                      <select
                        ...
                        className="w-full rounded-lg border bg-[var(--surface-2)] px-2.5 py-2.5 text-xs font-medium text-[var(--text-primary)] focus:outline-2 focus:outline-offset-2 focus:outline-[var(--accent)]"
                        style={{ borderColor: 'var(--border)' }}
                      >
                  ...
                  >
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-3 border-t pt-3 text-xs" style={{ borderColor: 'var(--gridline)' }}>
                      <div>
        ...
        <Card
          title="At a glance"
          subtitle="Same 2026 tax engine and metro benchmark costs as the calculator"
        >
                ...
                <tr className="text-xs text-[var(--text-muted)]">
                  ...
                  {results.map((result, index) => (
                    <th
                      ...
                      className="pb-3 px-3 text-right font-medium"
                    >
          ...
          <p className="mt-4 border-t pt-3 text-xs leading-5 text-[var(--text-muted)]" style={{ borderColor: 'var(--gridline)' }}>
            Comparison uses each metro’s built-in baseline living-cost estimates for the selected housing tier. Manual cost edits from the main calculator are not carried into this view so the scenarios stay easy to compare.
          </p>
        </Card>
      </main>
    </div>
